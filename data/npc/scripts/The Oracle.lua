...
function thinkCallback(cid)
	if math.random(300) == 1 then
		npcHandler:say("Harr! Wanna go off this island?")
	end
	return true
end
...
function creatureSayCallback(cid, type, msg)
	...
	elseif Topic[cid] == 1 then
		if msgcontains(msg, "yes") then
			npcHandler:say("Take a hint from Oracle. Make sure y'dun leave here before yer equipped well. Ya went for rob- err, shopping and got stuff like rope, armor, and weap'n?", cid)
			Topic[cid] = 2
