function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if getPlayerStorageValue(cid, config.bstorage) < 1 then
		return doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE, "Deathstrike defends his belongings and will not let you open his chest.")
	end
	...
	if getPlayerStorageValue(cid, config.storage) >= os.time() then
		...
		return doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE,"You have found 2 major crystalline tokens, 1 gill necklace, 1 mucus plug and 7 green crystal shards, wait "..config.hours.." hours to use this again.")
end