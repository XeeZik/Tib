function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if getPlayerStorageValue(cid, config.bstorage) < 1 then
		return doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE, "Abyssador defends his belongings and will not let you open his chest.")
	end
	...
	if getPlayerStorageValue(cid, config.storage) >= os.time() then
		...
		return doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE,"You have found 2 major crystalline tokens, 1 prismatic ring, 1 mucus plug and 10 violet crystal shards, wait "..config.hours.." hours to use this again.")
end