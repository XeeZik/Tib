function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if getPlayerStorageValue(cid, config.exaust) >= os.time() then
		return doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE, "Tanjis fights for his belongings and will not let you have them.")
	end
		...
		return doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE,"You have found 1 trophy of tanjis, 20 platinum coins, 1 necklace of the deep, 1 warrior shield and 1 tanjis sight.")

		elseif randomitem == 2 then
		doPlayerAddItem(cid, 2144, 10) -- 10 black pearl	
		setPlayerStorageValue(cid, config.storage, 2)
		setPlayerStorageValue(cid, config.exaust, os.time()+config.hours*3600)
		return doPlayerSendTextMessage(cid, MESSAGE_EVENT_ADVANCE,"You have found 1 trophy of tanjis, 20 platinum coins, 1 necklace of the deep, 1 warrior shield and 10 black pearl.")
	end
end