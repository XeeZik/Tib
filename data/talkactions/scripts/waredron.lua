function onSay(cid, words, param)
	...
	if param:find('invite') == 1 and 3 then
		...
		if player == nil or tonumber(minutos) == nil or player == '' or tonumber(minutos) == 0 then
			for i = 1, #status do
				doPlayerSendTextMessage(cid,status[i],"[War Anti Entrosa] Use: !waredron \"invite NOME_DE_UM_MEMBRO_INIMIGO MINUTOS_DE_WAR (EX: !waredron \"invite String 40 - invitar para uma war de 40 minutos)") 
			end
			return false
		end
		...
		for i = 1, #status do
			...
			if (getPlayerGuildLevel(tid) == 3) then
				doPlayerSendTextMessage(tid, status[i],"[War Anti Entrosa] Sua guild recebeu um convite da guild "..inguild.." para "..tonumber(minutos).." minutos na Area de Edron x Cormaya, voce tem 1 minuto para usar !waredron \"accept para aceitar ou !waredron \"reject para rejeitar!.")
			else
				doPlayerSendTextMessage(tid, status[i],"[War Anti Entrosa] Sua guild recebeu um convite de war da guild "..inguild.." para "..tonumber(minutos).." minutos na Area de Edron x Cormaya!.")
			end
		end
	...
	elseif param:lower():find('accept') == 1 then
		...
		if gid == guildtwo then
			if getGlobalStorageValue(config.usando) == 2 then
				doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR,"[War Anti Entrosa] Voce ja aceitou o convite e a war comeceu, para entrar na area basta usar !waredron \"join")
			elseif getGlobalStorageValue(config.usando) == 1 then
				...
				for i, tid in ipairs(players) do
					if getPlayerGuildId(tid) == guildtwo or getPlayerGuildId(tid) == guildone then
						for i = 1, #status do
							doPlayerSendTextMessage(tid, status[i],"[War Anti Entrosa] O acesso para sua guild foi liberado, voce tem 10 minutos para usar !waredron \"join")
						end
					end
				end
			end
	...
	elseif param:lower():find('stop') == 1 then
		...
		elseif gid == guildtwo then
			if getGlobalStorageValue(config.acc2storage) ~= 1 and getGlobalStorageValue(config.acc1storage) == 1 then
				doPlayerSendTextMessage(cid,MESSAGE_INFO_DESCR,"[War Anti Entrosa] Voce ja pediu um stop, porem o outro lider ainda nao aceitou usando !waredron \"stop")
